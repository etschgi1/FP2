...
def main() -> None:
    ...
    lateral = lt.Student([10.0, 9.5, 10.0])
    distance = lt.Student([3.0, 3.5, 3.0])
    angle = lt.Student([22, 20, 25])
    ...
    radius = calculate_radius_wavefront(
        lateral.mean * 1e-3, distance.mean * 1e-3, angle.mean * lt.np.pi / 180, lt.u.ufloat(537, 18) * 1e-9
    )
    print(f"{radius=}")
